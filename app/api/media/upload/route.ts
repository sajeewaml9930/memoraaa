...
} from "@/app/lib/image-processor";
import {
  compressVideo,
  extractVideoThumbnail,
  getVideoDuration,
} from "@/app/lib/video-processor";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const caption =
      typeof captionValue === "string" ? captionValue.trim().slice(0, 200) : "";
    const keepOriginalQuality =
      keepOriginalQualityValue === "true" || keepOriginalQualityValue === "1";
    const normalizedMood =
      typeof moodValue === "string" ? moodValue.trim().toLowerCase() : null;
    const isVideoUpload =
      file instanceof File && file.type.toLowerCase().startsWith("video/");
    ...
    if (!isImageUpload && !isVideoUpload && !isAudioUpload) {
      return NextResponse.json(
        {
          error:
            "Unsupported file type. Please upload an image, video, or audio recording.",
        },
        { status: 400 },
      );
    }
    ...
    if (isVideoUpload && !isValidVideoType(normalizedFileType)) {
      return NextResponse.json(
        {
          error:
            "Unsupported video type. Please upload MP4, MOV, WebM, or AVI.",
        },
        { status: 400 },
      );
    }
    ...
    if (isAudioUpload && !isValidAudioType(normalizedFileType)) {
      return NextResponse.json(
        {
          error:
            "Unsupported audio type. Please upload MP3, M4A, AAC, WAV, WebM, or OGG audio.",
        },
        { status: 400 },
      );
    }
    ...
    if (isVideoUpload) {
      ...
      processedBuffer = await compressAudio(fileBuffer, {
        bitrate: "96k",
        format: "mp3",
      });
      duration = Math.max(
        1,
        Math.round(await getAudioDuration(processedBuffer)),
      );
    } else {
      const orientedImageBuffer = await normalizeImageOrientation(fileBuffer);
      processedBuffer = keepOriginalQuality
        ? orientedImageBuffer
        : await compressImage(orientedImageBuffer);
      thumbnailBuffer = await generateThumbnail(orientedImageBuffer);
    }
    ...
    const memoryType = isVideoUpload
      ? "video"
      : isImageUpload
        ? "photo"
        : "voice";
    const savedThumbnailPath = thumbnailBuffer
      ? (() => {
          const encryptedThumbnail = encryptData(
            thumbnailBuffer,
            user.encryptionKey,
          ) as Buffer;
          return saveFile(
